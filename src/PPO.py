...
class ActorCritic(nn.Module):
    def __init__(
        self, layer_array, n_obs, n_actions, orthogonal, init_variance, device
    ):
...
class PPO:
    def __init__(
        self, env_cfg_file, train_cfg_file, run_name="None", checkpoint="None"
    ):
        ...
        self.vecenv = isaacgym_task_map[env_cfg["name"]](
            env_cfg,
            env_cfg["rl_device"],
            env_cfg["sim_device"],
        ...
        self.time_per_update = (
            env_cfg["sim"]["dt"] * self.rollout_steps
        )  # rollout Sim time before each NN update

        self.action_var_decay = train_cfg["action_init_var"] / self.total_updates
        self.ac = ActorCritic(
            train_cfg["nn_layer_connections"],
            self.num_obs,
            self.num_actions,
        ...
        self.obs_buf = torch.zeros(
            (self.rollout_steps, self.num_envs, self.num_obs),
            device=self.device,
            dtype=torch.float,
        )
        self.reward_buf = torch.zeros(
            (self.rollout_steps, self.num_envs), device=self.device, dtype=torch.float
        )
        ...
        self.action_buf = torch.zeros(
            (self.rollout_steps, self.num_envs, self.num_actions),
            device=self.device,
            dtype=torch.float,
        )
        self.log_prob_buf = torch.zeros(
            (self.rollout_steps, self.num_envs), device=self.device, dtype=torch.float
        )
        ...
        self.next_reset_buf = torch.ones(
        ...
        while self.update_step < self.total_updates:
            ...
            for step in range(self.rollout_steps):
                ...
                with torch.no_grad():
                    ...
                    action, prob_dist = select_action_multinormaldist(
                        self.ac.actor(self.obs_buf[step]), self.ac.actor_variance
                    )
            ...
            with torch.no_grad():
                self.value_buf[self.rollout_steps] = self.ac.critic(
                    self.next_obs_buf
                ).squeeze()
            ...
            for step in reversed(range(self.rollout_steps)):
                delta = (
                    self.reward_buf[step] + self.gamma * self.value_buf[step + 1] * (1 - self.reset_buf[step + 1])- self.value_buf[step]
                )
                gae_buf[step] = (
                    delta + self.gamma * self.lambda_ * gae_next * (1 - self.reset_buf[step + 1])
                )
            ...
            if not self.checkpoint:
                ...
                for k in range(self.num_epoch):
                    ...
                    for mb in range(self.rollout_steps // self.minibatch_size):
                        minibatch_indicies = shuffled_indicies[
                            mb * self.minibatch_size : (mb + 1) * self.minibatch_size
                        ]
                        _, prob_dists = select_action_multinormaldist(
                            self.ac.actor(self.obs_buf[minibatch_indicies]),
                            self.ac.actor_variance,
                        )
                        log_probs_new = prob_dists.log_prob(
                            self.action_buf[minibatch_indicies]
                        )

                        values = self.ac.critic(
                            self.obs_buf[minibatch_indicies]
                        ).squeeze()
                        log_prob = self.log_prob_buf[minibatch_indicies]
                        returns = return_buf[minibatch_indicies]
                        advantage = gae_buf[minibatch_indicies]
                        self.update_net(
                            log_prob, log_probs_new, values, returns, advantage
                        )

            # End of update tasks
            self.ac.actor_variance -= self.action_var_decay
            self.tb.add_scalar(
                "Hyperparam/actor_variance",
                self.ac.actor_variance.mean(),
                self.global_step,
            )
            if self.anneal_lr:
            ...
            self.tb.add_scalar(
                "Score/episode_reward", float(mean_ep_reward), self.global_step
            )
            finished_episodes = 0
            episode_reward_sum = 0
            ...
            self.tb.add_scalar(
                "Score/time_average_reward",
                float(reward_time_average),
                self.global_step,
            )
            reward_sum = 0
            ...
            if self.update_step % self.eval_freq == 0:
                ...
                print(
                    ...
                    + ", Network updates:"
                    + str(self.update_step)
                    + ": Score: "
                    + str(round(score.item(), 2))
                    + ", Action Variance: "
                    + str(round(self.ac.actor_variance.data.mean().item(), 2))
                )
                if score > best_score:
                    print("Saving best weights ")
                    best_score = score
                    torch.save(
                        self.ac.state_dict(), self.log_dir + "/" + "best_weigths"
                    )
